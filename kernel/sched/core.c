...
int get_nohz_timer_target(int pinned)
{
	...
	if (pinned || !get_sysctl_timer_migration() || (!idle_cpu(cpu) && is_housekeeping_cpu(cpu)))
		return cpu;
 ...
 */
static void migrate_tasks(unsigned int dead_cpu, bool migrate_pinned_tasks)
{
	...
	for ( ; ; ) {
		...
		dest_cpu = select_fallback_rq(dead_cpu, next, false);
		raw_spin_unlock(&rq->lock);
