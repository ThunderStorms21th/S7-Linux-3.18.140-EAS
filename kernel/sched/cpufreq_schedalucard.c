...
static struct acgov_tunables *acgov_tunables_alloc(struct acgov_policy *sg_policy)
{
	...
	if (tunables) {
		...
		for (i = 0; i < tunables->nelements; i++) {
			if (policy->cpu < 4) {
			tunables->up_rate_limit_us[i] = UP_RATE_LIMIT_US_LC;
			tunables->down_rate_limit_us[i] = DOWN_RATE_LIMIT_US_LC;
			} else {
			tunables->up_rate_limit_us[i] = UP_RATE_LIMIT_US_BIGC;
			tunables->down_rate_limit_us[i] = DOWN_RATE_LIMIT_US_BIGC;
			}
