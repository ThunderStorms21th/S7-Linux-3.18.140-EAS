...
#define LATENCY_MULTIPLIER_DOWN_LC		(60000)
#define LATENCY_MULTIPLIER_DOWN_BC		(60000)
...
static int cpufreq_pixel_smurfutil_gov_cb(struct cpufreq_policy *policy,
				unsigned int event)
{
	switch(event) {
...
static struct cpufreq_governor pixel_smurfutil_gov = {
	.name = "pixel_smurfutil",
	.governor = cpufreq_pixel_smurfutil_gov_cb,
	.owner = THIS_MODULE,
};
